                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="table-content table-responsive">
                                <table>
                                    ...
                                    <tbody>
                                                    <%
                                                        // Retrieve the cartItems list from the model
                                                        List<demorkmjava.dto.CartItemDto> cartItems =
                                                            (List<demorkmjava.dto.CartItemDto>) request.getAttribute("cartItems");

                                                        double grandTotal = 0;

                                                        // Loop through the cart items and render the details
                                                        if (cartItems != null && !cartItems.isEmpty()) {
                                                            for (demorkmjava.dto.CartItemDto cartItem : cartItems) {
                                                                demorkmjava.model.Cart cart = cartItem.getCart();
                                                                demorkmjava.model.Product product = cartItem.getProduct();

                                                                double total = product.getPrice() * cart.getQuantity();
                                                                grandTotal += total;
                                                    %>
                                                    <tr id="cart-item-<%= cart.getId() %>">
                                                                <td class="product-thumbnail">
                                                                    <a href="#"><img src="<%= product.getImageUrl() %>" alt="<%= product.getName() %>"></a>
                                                                </td>
                                                                <td class="product-name">
                                                                    <a href="#"><%= product.getName() %></a>
                                                                </td>
                                                                <td class="product-price">
                                                                    <span class="amount">₹ <%= String.format("%.1f", product.getPrice()) %> Lakhs</span>
                                                                </td>
                                                                <td class="product-quantity">
                                                                    <div class="quantity-range">
                                                                        <input class="input-text qty text" type="number" step="1" title="Qty" size="4"
                                                                            id="input-quantity-<%= cart.getId() %>" value="<%= cart.getQuantity() %>" min="1">
                                                                    </div>
                                                                </td>
                                                                <td class="product-subtotal">
                                                                    ₹ <%= String.format("%.1f", total) %> Lakhs
                                                                </td>
